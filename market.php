...
<body>
    ...
    $sql = mysqli_query($con,"SELECT id_user,username,uang,j_knight FROM t_user WHERE username = '$username'");
    ...
    <P>Jumlah Knight Saat Ini : <input type="text" name="sisa" value="<?php echo $a['j_knight']?>" readonly></p> 
    <?php 
$query = mysqli_query($con,"SELECT * FROM t_market WHERE barang = 'knight'") or die(mysqli_error($con));
...
if(isset($_POST['submit'])){
    ...
    if($uang > $harga){
        ...
        if($delete){
        $sql = mysqli_query($con,"UPDATE t_user SET j_knight = $knight + $jumlah WHERE id_user = '$id_user'");    
